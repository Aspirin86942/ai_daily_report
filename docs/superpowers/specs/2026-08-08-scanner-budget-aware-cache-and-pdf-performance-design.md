# Scanner 预算感知解析 + 缓存瘦身 + 激进性能 设计规格

> 状态：Ready for implementation
> 日期：2026-08-08
> 决策范围：Rust scanner core 的解析预算、缓存、快照、PDF/Office worker 与性能调优
> 首要目标：以真实目录证据驱动，消除「解析大量文件却只进少量上下文」的浪费，并收敛 PDF 冷扫瓶颈
> 不涉及：Rust CLI JSON wire contract、报告 schema/模板、LLM 行为、office backend 迁移

## Problem Statement

### 真实语料与实测证据（2026-08-08，`D:\01- 工作`）

对真实工作目录 `D:\01- 工作`（3293 文件 / 13.4GB）做清点与受控实测：
...
1. **解析浪费（核心）**：`run.rs` 先对**所有 cache miss 文件**并行解析（office/PDF 每个起独立子进程），`compressor.rs` 再按全局预算 `total_max_chars` 把放不下的文件 `Omit` 掉。实测 136 个解析中只有 14 个进上下文，**90% 的解析和缓存是纯浪费**。大窗口（90d/全年）会为「塞进 ~14 个文件」解析 273~823 个慢 PDF。
2. **缓存臃肿**：`parse_cache` 对每个成功解析的文件都写一行内容缓存，实测 136 行而只有 14 行（进上下文的）下一轮会被用到。存储与写放大 9x。
3. **PDF 是唯一大瓶颈**：office 已吃满 Rust 红利（15ms 级），PDF 走 Python worker 平均 2.5s（慢 100 倍），且 90d 窗口实测会**卡死 >5min**（部分病态大 PDF）。PDF→Rust 迁移已在 2026-08-07 被门禁否决，PDF 短期只能留在 Python。
4. **温扫 discovery 成本**：温扫 347ms 中 272ms 是 `WalkDir` 全量遍历 + 逐文件 `canonicalize`+stat，未利用已存储的 `context_runs.final_context`。
5. **并行度与查询**：`max_workers=4` 固定；`attach_cache_evidence` 逐文件单条 SQLite 查询（N 次往返）。

### 目标与成功标准

1. 90d 窗口冷扫从「卡死 >5min」→ 目标 **≤40s**（预算早停核心收益）。
2. 月报 30d 冷扫约 1min → 目标 **≤20s**。
3. 昂贵档（PDF/legacy，~2.5s/个）不再为装不下的文件解析：**被预算省略的昂贵文件全部 `not_parsed`（不启动进程）**，实测昂贵档预算外解析数 ≈ 0（当前 90d 会解析数百个 PDF）。便宜档（office/text）全量解析成本有界且可忽略（90d 窗口约 1.5s）。
4. `parse_cache` 行数从 N 降到 **K（included 数）**，DB 体积明显下降。
5. 温扫无变化命中快照 → 目标 **≤150ms**（从 347ms）。
6. 现有 fixed-corpus 门禁保持全绿；新增「真实目录 90d 冷扫有界完成」回归（≤40s，无 timeout/error）。
7. 上下文质量不降：included 文件集合与内容保持确定性；未解析文件有明确审计标记。
...
- 昂贵档（PDF + legacy xls，~2.5s/个）：按呈现顺序**分块**处理——每次并行解析下一个 `max_workers` 个昂贵文件，逐块渲染并入预算；**剩余预算装不下下一块时停止**，其余昂贵文件标 `not_parsed`。
...
- 省略文件摘要的 `input_chars` 用 **size_bytes 近似**（用户已拍板），渲染时标注 `~` 近似记号。
- `source_file_count`、`omitted_file_count` 语义不变（包含未解析文件）。

**边界 PDF 防护**：对昂贵文件用保守门控 `剩余预算 ≥ 该文件 excerpt 上限`（excerpt 上限按类型已知）预判，避免对必然装不下的文件启动进程。

**与并行通道的关系**：便宜档仍走现有 `ParserScheduler.parse_planned_files`（rayon 全量）；昂贵档改为分块有界并行，块间渲染记账。允许最多一块（≤`max_workers` 个文件）的边际浪费，换取有界并行与早停。

### Part 2（P1）：缓存瘦身到 included-only

- `finalize` 的 `cache_writes` 只包含**最终进入上下文（included）**文件的记录（action=keep/compress/metadata_only），不再对每个成功解析文件写缓存。
- 温扫复用不受影响：下一轮需要的正是 included 文件的内容。
- 预算漂移导致以前 omitted 文件变成 included → cache miss → 重解析，语义正确。
- v2 迁移时清理旧库中非 included 的 `parse_cache` 行。
...
### Part 4（P1，激进 PDF）：批量 worker + pypdfium2 门禁评估

#### 4a 批量 PDF worker
- 一次性进程处理一批 PDF（列表输入、列表输出），摊薄 Python 解释器启动 + pdfplumber import（占单文件 2.5s 中约 0.5~1s）。
- 分块大小与 `max_workers` 对齐；批内单文件超时仍逐文件判定。
- **worker contract 升 `WORKER_CONTRACT_VERSION` 到 v2**，新契约独立测试。
- 崩溃隔离降级：批失败时按块回退到逐文件路径，保留现有隔离。

#### 4b pypdfium2 门禁评估（重开 2026-08-07「PDF 维持 pdfplumber」边界）
- pypdfium2 已在依赖中（`pypdfium2-5.12.1`），**无需新增依赖**，仅做候选评估。
...
- **两项都通过才替换生产 `document_parser.py` 的 PDF 分支**；任一不通过则维持 pdfplumber，评估结果存档（不修改生产代码）。
...
2. `parse_cache` 语义收紧为「仅 included 文件」，迁移时清理旧库非 included 行。
...
- 早停、快照命中、缓存瘦身都必须保持**确定性**：同输入（目录状态 + profile）同输出。
...
- 缓存瘦身：断言 `parse_cache` 只含 included 文件；预算漂移后 included 集合变化时温扫正确重解析。
...
- [ ] 月报 30d 冷扫 ≤20s。
- [ ] 昂贵档解析不超预算：被预算省略的昂贵文件全部 `not_parsed`（不启动进程），实测昂贵档预算外解析数 ≈ 0；便宜档全量解析成本有界（90d 约 1.5s），实测记录。
- [ ] `parse_cache` 只含 included 文件，DB 体积下降（实测对比）。
- [ ] 温扫无变化命中快照，≤150ms。
- [ ] 早停与快照输出确定；未解析文件审计标记正确。
- [ ] fixed-corpus 门禁全绿；Rust workspace 测试、pytest 全绿、doctor --strict 通过。
- [ ] pypdfium2 门禁结论存档（通过→替换；不通过→维持 pdfplumber），生产代码按门禁结果落地。
...
5. 跨载体报告事务、Web/GUI/daemon/queue。
6. 非本 spec 提及的 parser backend 替换。
...
- **ID-4 parse_cache 仅 included**：直接消除缓存臃肿，温扫复用不受损。
- **ID-5 快照指纹**：目录状态指纹 + profile hash 双键，确定性高、实现小。
- **ID-6 分块有界并行**：昂贵档按 `max_workers` 分块，允许一块边际浪费换取早停与并行。
- **ID-7 批量 worker 带批失败回退**：保留崩溃隔离，收益在摊薄 Python 启动+import。
- **ID-8 PDF 门禁独立于生产**：候选不达标零生产影响，复用既有门禁方法。
- **ID-9 v2 迁移事务化**：沿用既有 user_version 机制，失败可回退。

## 建议实施顺序

1. Part 1 预算早停（含 schema 不需要先动）+ 对应测试与真实目录回归。
2. Part 2 缓存瘦身 + v2 迁移（含旧行清理）。
3. Part 3 快照快速路径 + v2 指纹存储。
4. Part 5 低风险调优（max_workers、批量 cache 查询）。
5. Part 4a 批量 PDF worker（contract v2）。
6. Part 4b pypdfium2 门禁评估 → 按门禁结果决定是否替换生产 PDF 分支。
...
| pypdfium2 候选翻车 | 门禁否决即维持 pdfplumber，零生产影响 |
| 早停/快照破坏确定性 | 同输入同输出测试；指纹不一致回退完整扫描 |
...
- 2026-08-08 用户确认：实施方向一（预算早停 + 缓存瘦身 + 快照）并加入激进 PDF 性能（批量 worker + pypdfium2 门禁评估）与低风险调优（max_workers 自动取值、批量 cache 查询）。
