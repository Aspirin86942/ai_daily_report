...
| summary 模式文本上限（周/月） | 2k | **100k** | 与日报统一，取消降档 |
| `document_excerpt_max_chars`（Office/PDF） | 6k | **100k** | |
...
1. 保持尾部优先：取最后 `limit − 64` 字符（逐字后缀，不回退行边界——日志为时间序尾部优先场景，头部省略标记已显式说明截断）；
2. 头部加省略标记：`…（已省略头部约 N 字符）…`。
