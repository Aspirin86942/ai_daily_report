...
class WorkItem(BaseModel):
    ...
    quantitative: Optional[str] = Field(default=None, description="量化指标 (如: 审计3个项目, 发现5个问题)")
...
class DailyReportData(BaseModel):
    ...
    yesterday_review: Optional[str] = Field(default=None, description="昨日计划完成情况评估")
...
class WeeklyReportData(BaseModel):
    ...
    category_summaries: List[CategorySummary] = Field(description="按类别汇总的工作列表")
    risks: List[RiskItem] = Field(default_factory=list, description="风险与问题列表")
    key_achievements: List[str] = Field(description="本周重点成果")
    next_week_plans: List[str] = Field(description="下周工作计划")
    missing_days: List[str] = Field(default_factory=list, description="缺失日报的日期列表")
...
class MonthlyReportData(BaseModel):
    ...
    category_summaries: List[CategorySummary] = Field(description="按类别汇总的工作列表")
    risks: List[RiskItem] = Field(default_factory=list, description="风险与问题列表")
    statistics: Dict[str, str] = Field(default_factory=dict, description="统计指标 (灵活 KV)")
