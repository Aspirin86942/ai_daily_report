...
def test_timer_covers_child_spawn_and_response_validation(tmp_path):
    ...
    def validator(raw: bytes) -> bool:
        # 校验器自身耗时 200ms；wall_ms >= 400 只有在校验在计时之内才成立
        time.sleep(0.2)
        return json.loads(raw)["ok"] == 1

    result = wall_clock_ms([sys.executable, str(script)], payload, response_validator=validator)
    elapsed = time.perf_counter() - started
    # 必须 >= child sleep(0.2) + validator sleep(0.2)（证明覆盖 response validation 之后）
    assert result.wall_ms >= 400, result.wall_ms
